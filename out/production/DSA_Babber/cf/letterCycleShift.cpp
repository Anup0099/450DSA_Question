...
void solve()
{
    string s;
    cin >> s;
    bool has_been_flippped_atLeast_once = false;
    bool start_flip = false, never_flip = false;

    for(int i = 0; i < s.length(); i++)
    {
        if (s[i] == 'a' && start_flip==true)
        {
            never_flip = true;
            start_flip = false;
            break;
        }
        if(s[i] != 'a' && never_flip==false){
            start_flip = true;
        }
        if (s[i] != 'a' && start_flip)
        {   s[i]--;
            has_been_flippped_atLeast_once = true;
        }
    }
    if(!has_been_flippped_atLeast_once){
        s.back()=='z';
    }
    cout<<s<<endl;
}
